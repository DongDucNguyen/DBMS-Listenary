...
-- =========================================================================================
-- FILE GỘP TẤT CẢ CODE MIGRATION TỪ ĐẦU (ALL-IN-ONE)
-- Chứa toàn bộ các tập lệnh theo thứ tự chuẩn xác để cập nhật DB từ cơ bản đến tối ưu nhất.
-- =========================================================================================
...
-- -----------------------------------------------------------------------------------------
-- PHẦN MỚI: TỪ FILE [fix_autoincrement.sql]
-- -----------------------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------------------
-- PHẦN MỚI: TỪ FILE [fix_schema_fk.sql]
-- -----------------------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------------------
-- PHẦN MỚI: TỪ FILE [fix_comment_proc.sql]
-- -----------------------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------------------
-- PHẦN MỚI: TỪ FILE [optimize_indexes.sql]
-- -----------------------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------------------
-- PHẦN MỚI: TỪ FILE [db_bug_fixes.sql]
-- -----------------------------------------------------------------------------------------

USE DBMS_Listenary;
SET FOREIGN_KEY_CHECKS = 0;

-- 1. FIX: Gộp 2 trigger BEFORE UPDATE của books (vì MySQL chỉ cho phép 1 trigger mỗi event/time)
DROP TRIGGER IF EXISTS trg_AfterBookUpdate;
DROP TRIGGER IF EXISTS trg_AfterBookApproval;
DROP TRIGGER IF EXISTS trg_BeforeBookUpdate;

DELIMITER $$
CREATE TRIGGER trg_BeforeBookUpdate
...
-- 2. FIX: Lỗi FK của user.authorId. Khi insert user mới chưa có authorId thì phải set = NULL thay vì 0
DROP TRIGGER IF EXISTS trg_BeforeUserInsert $$
CREATE TRIGGER trg_BeforeUserInsert
...
-- 3. FIX: Xóa trigger trg_AfterListeningInsert vì sp_UpsertListeningHistory ĐÃ tăng viewCount, nếu để trigger sẽ bị double count.
DROP TRIGGER IF EXISTS trg_AfterListeningInsert $$
DELIMITER ;

-- 4. FIX: Đổi UNIQUE index ở comments thành NORMAL index vì 1 user có thể comment nhiều lần trên 1 sách
SET @s = IF((SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='comments' AND INDEX_NAME='idx_comments_user_book') > 0, 'ALTER TABLE comments DROP INDEX idx_comments_user_book', 'SELECT 1');
PREPARE _st FROM @s; EXECUTE _st; DEALLOCATE PREPARE _st;

SET @s = IF((SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='comments' AND INDEX_NAME='idx_comments_user_book') = 0, 'ALTER TABLE comments ADD INDEX idx_comments_user_book (userId, bookId)', 'SELECT 1');
PREPARE _st FROM @s; EXECUTE _st; DEALLOCATE PREPARE _st;

-- 5. FIX: Phục hồi lại FK fk_us_plan bị thiếu sau khi optimize indexes
SET @s = IF((SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='usersubscriptions' AND CONSTRAINT_NAME='fk_us_plan') = 0, 'ALTER TABLE usersubscriptions ADD CONSTRAINT fk_us_plan FOREIGN KEY (planId) REFERENCES subscriptionplans(id) ON UPDATE CASCADE ON DELETE RESTRICT', 'SELECT 1');
PREPARE _st FROM @s; EXECUTE _st; DEALLOCATE PREPARE _st;

-- 6. FIX: Đảm bảo trigger Delete cascade không xung đột với FK Cascade
-- FK user -> comments, userfavorites, usersubscriptions, listeningaudiobook đã là ON DELETE CASCADE
-- Nên trigger trg_AfterUserDelete chỉ là dư thừa và gây lỗi "Can't update table in stored function/trigger"
DROP TRIGGER IF EXISTS trg_AfterUserDelete;
